...
section ``Class File Format Extensions'' of the JSR 308 design document~\cite{JSR308},
...
\paragraph{Bound definitions}

The \bnfnt{bound-definition} production defines annotations on a bound of a
...
\paragraph{Implements and extends definitions}

The \bnfnt{extends-definition} and \bnfnt{implements-definition} productions
...
\paragraph{Return definition}

A return definition defines the annotations on the return type of a method
declaration.
...
\subsubsection{Code annotations\label{code-annotations}}
...
\bnfnt{variable-declaration} rule.

Because of the differences between Java source code and \code{.class} files,
the syntax for specifying these annotations is different for \code{.class} files
...
\paragraph{Code annotations grammar}
...
Additionally, an annotation file supports using a path through the AST (abstract
...
are often used in annotation definitions so for ease of use are not required to
...
\subsubsection{Compound type annotations\label{compound-type-annotations}}
