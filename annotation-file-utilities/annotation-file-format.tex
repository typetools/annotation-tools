...
\bnfnt{call-location} ::= \\
\qquad    \bnfcmt Bytecode offset: the first integer is the offset field and the optional \\
\qquad    \bnfcmt second integer is the zero-based index of the runtime type argument~\cite{JSR308-webpage-201110}. \\
\qquad    (\bnflit{\#} \bnfnt{integer}) \\
\qquad    \bnfcmt Source code index: the integer is the zero-based index of the method call \\
\qquad    \bnfcmt within the method and the second integer is the zero-based index of the \\
\qquad    \bnfcmt runtime type argument~\cite{JSR308-webpage-201110}. \\
\qquad    \bnfor{} (\bnflit{*} \bnfnt{integer}) \\
\\
\bnfnt{call} ::= \\
\qquad    \bnflit{method-call} \bnfnt{call-location} \bnflit{:} \lineend \\
\qquad    \bnfnt{expression-annotations}* \\
\\
...
\\
\bnfnt{reference} ::= \\
\qquad    \bnflit{method-reference} \bnfnt{reference-location} \bnflit{:} \bnfnt{type-annotation}* \lineend \\
\qquad        \bnfnt{typearg-definition}* \\
\qquad        \bnfnt{typearg-bound}* \\
%\qquad        \bnfnt{return-type}? \\
\qquad        \bnfnt{parameter-definition}* \\
\qquad        \bnfnt{variable-definition}* \\
\qquad        \bnfnt{expression-annotations}*
...
\\
\bnfnt{lambda} ::= \\
\qquad    \bnflit{lambda} \bnfnt{lambda-location} \bnflit{:} \bnfnt{type-annotation}* \lineend \\
\qquad        \bnfnt{typearg-definition}* \\
\qquad        \bnfnt{typearg-bound}* \\
%\qquad        \bnfnt{return-type}? \\
\qquad        \bnfnt{parameter-definition}* \\
...
\qquad    \bnflit{type-argument} \bnfnt{integer} \bnflit{:} \bnfnt{type-annotation}* \lineend \\
\qquad    \bnfnt{compound-type}*
\\
\bnfnt{typearg-bound} ::= \\
\qquad    \bnfcmt The integers are respectively the argument and bound indexes of \\
\qquad    \bnfcmt the type argument bound~\cite{JSR308-webpage-201110}. \\
\qquad    \bnflit{bound} \bnfnt{integer} \bnflit{\&} \bnfnt{integer} \bnflit{:} \bnfnt{type-annotation}* \lineend \\
\qquad    \bnfnt{compound-type}*
\end{tabbing}
