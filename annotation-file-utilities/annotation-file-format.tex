...
\qquad        \bnfnt{insert-typecast-definition}* \\
\\
...
\qquad    \bnfnt{type-argument-or-array-definition}* \\
\\
\bnfnt{insert-typecast-definition} ::= \\
\qquad    \bnfcmt \bnfnt{ast-path} is described in Section~\ref{ast-paths}. \\
...
insertion: one set around the expression to be cast, and other set around the
cast and the expression. This is to limit the cast to the entire expression, but
nothing more. These parentheses may be unnecessary and can be removed by the
programmer afterward. Also note that a cast can inserted on any expression, not
just the lowest expression in the AST. For example, a cast could be inserted on
the expression \code{i + j}, the literal \code{i}, and/or the literal \code{j}.
...
\qquad    \bnfor{} \bnfnt{whildcard-tree} \\
\\
