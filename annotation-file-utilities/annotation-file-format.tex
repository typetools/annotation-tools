...
\qquad    ( \bnfnt{annotation-definition} | \bnfnt{class-definition} ) *
\end{tabbing}
...
\qquad    | \bnflit{invisible} \bnfcmt Equivalent to \code{@Retention(CLASS)} in source \\
\qquad    | \bnflit{source} \bnfcmt Equivalent to \code{@Retention(SOURCE)} in source
\end{tabbing}
...
and methods in the class.  Program elements that are not annotated may be
present or may be omitted.  Class definitions
...
% LocalWords:  enum arr quined int pt instanceof
