...
import scenelib.annotations.util.coll.WrapperMap;
    ...
    cut.accept(
        new SimpleTreeVisitor<Void, ASTRecord>() {
          ...
          public Void visitCase(CaseTree node, ASTRecord rec) {
            Kind kind = node.getKind();
            saveAll(org.checkerframework.javacutil.TreeUtils.caseTreeGetExpressions(node), rec, kind, ASTPath.EXPRESSION);
